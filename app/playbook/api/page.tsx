...
export default function ApiPlaybookPage() {
    ...
    <main id="playbook" className="px-3 pt-24 md:pt-28 pb-12 max-w-7xl mx-auto">
      <header className="text-center py-4 mb-6 max-w-4xl mx-auto">
        ...
        <p className="text-sm md:text-base text-muted-foreground max-w-4xl mx-auto leading-relaxed font-light animate-in fade-in slide-in-from-bottom duration-1000 delay-200">
          Comprehensive implementation guidance and strategic patterns for REST and GraphQL API test automation at scale.
        </p>
        {mainTitle && (
      ...
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_22rem] gap-6 items-start relative">
        {/* Floating Collapse/Expand All Button - Fixed Left Side Center */}
        <div
          onClick={toggleAllSections}
          className="fixed left-6 top-1/2 -translate-y-1/2 z-50 px-4 py-3 rounded-full bg-gradient-to-br from-blue-500 via-purple-600 to-pink-500 shadow-lg cursor-pointer transition-all duration-300 hover:shadow-xl hover:scale-105 flex items-center justify-center group hidden lg:flex border-2 border-white/20 backdrop-blur-sm"
        ...
        >
          ...
          <span className="text-white font-medium text-sm whitespace-nowrap">
            {sections.every(section => collapsedSections.has(section.heading.slug)) ? "Expand All" : "Collapse All"}
          </span>
        </div>
            ...
            {sections.map((section, index) => {
              ...
              return (
                <div key={section.heading.slug} className="section-container relative">
                  ...
                  {/* Collapsible Section Header - Same for all sections */}
                  <div
